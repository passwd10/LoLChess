...
public class Elise extends Champion {

    public Elise(String name, String chamClass, String tribe, int tier, int hp, int mp, int power, double attackSpeed, int armor, int gold, int grade) {
    ...
    public int useSkill(AllUnit[] target) {
        if (getMp() >= getMAX_MP()) {
            ...
            if (getGrade() == 1) { //1성일때
                target[0].setHp(target[0].getHp()-getPower()*0.6);
                setHp(getHp()+getPower()*0.6);
            }
            if (getGrade() == 2) { //2성일때
                target[0].setHp(target[0].getHp()-getPower()*0.9);
                setHp(getHp()+getPower()*0.9);
            }
            if (getGrade() == 3) { //3성일떄
                target[0].setHp(target[0].getHp()-getPower()*1.2);
            ...
            System.out.println("/ MP " + getMp() + " ]");
            System.out.println("[Skill] 거미 형태 "); //150, 275, 400
            System.out.println("↓↓↓↓↓↓↓↓↓↓↓↓");
        }
        return 0;
    }
