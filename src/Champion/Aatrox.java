...
public class Aatrox extends Champion {
    ...
    public Aatrox(String name, String chamClass, String tribe, int tier, int hp, int mp, int power, double attackSpeed, int armor, int gold, int grade) {
    ...
    public int useSkill(AllUnit[] target) {
        if (getMp() >= getMAX_MP()) {
            ...
            if (getGrade() == 1) { //1성일때
                ...
                target[0].setHp(target[0].getHp()-950);
            }
            setMp(0);
