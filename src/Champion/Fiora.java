...
public class Fiora extends Champion {
    ...
    public Fiora(String name, String chamClass, String tribe, int tier, int hp, int mp, int power, double attackSpeed, int armor, int gold, int grade) {
    ...
    public void useSkill(AllUnit[] target) {
        if (getMp() >= getMAX_MP()) {
            ...
            if (getGrade() == 2) { //2성일때
                target[0].setHp(target[0].getHp()-250);
            }
            if (getGrade() == 3) { //3성일떄
            ...
            System.out.println("/ MP " + getMp() + " ]");
            System.out.println("[Skill] 응수 "); //150, 275, 400
            System.out.println("↓↓↓↓↓↓↓↓↓↓↓↓");
        }
    }
