...
public class Brand extends Champion {
    ...
    public Brand(String name, String chamClass, String tribe, int tier, int hp, int mp, int power, double attackSpeed, int armor, int gold, int grade) {
    ...
    public int useSkill(AllUnit[] target) {
        if (getMp() >= getMAX_MP()) {
            ...
            if (getGrade() == 2) { //2성일때
                target[0].setHp(target[0].getHp()-375);
            }
            if (getGrade() == 3) { //3성일떄
                target[0].setHp(target[0].getHp()-550);
            }
            setMp(0);
